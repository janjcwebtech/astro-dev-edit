...
- `ApplyOp` gains a `usage` target carrying `{usageId, prop | slot}`; `UsageProp` gains `editable: boolean` plus a `refusal` naming why not, so the client renders a reason rather than guessing.
