...
- ~~**Whether the legacy annotation read path can go**~~ — it went, in P7. It depended on the parity counter showing zero gaps across both fixtures. It showed zero on every page driven, and the loc comparison says why: across five Astro 5.18.2 routes, **all 1,721** of Astro's own annotations are reproduced byte-identically by the transform, and we annotate 75 *more* (`<html>`, `<head>` and head `<meta>`/`<link>`/`<title>`, which Astro's compiler skips and we deliberately do not). The read path was a strict subset — and, driven live in P7, an empty one: no legacy annotation survives to hover time on either major.
...
- [x] **P7** — committed fixture site with a genuine 3-deep chain; real-site pass on both fixtures. The fixture is `Marketing.astro` → `SiteLayout` → `FeatureSection` → `FeatureCard`, served at `/marketing`: every word is written in `Marketing.astro` and none renders there. Depth 4 from the route, which is deeper than the real site's deepest — `01-example-site-2`'s homepage runs 890 elements at depth 1, 329 at depth 2, 12 at depth 3, none beyond. Pinned by `composition-render.test.ts`, including the half a real site meets first: a prop **forwarded** through an intermediate component is `read-only`/`untraced` at every hop that merely passes it on, and `editable` exactly once, where the literal is written. The pass found three things, all filed: #73 (the chain states a prop is `editable` and renders it as inert text — the biggest gap, and the shape every layout/section/card page has), #74 (29 identical `slot default · read-only` rows on a real layout) and #72 (`data-atx-file` is an absolute path: a home-directory leak as well as 147 KB of a 895 KB page). Report: `internal-documentation/reports/p7-real-site-pass/`
- [x] **P7** — remove the legacy annotation read path, parity counter as evidence. The client reads `data-atx-*` only; `annotationGaps()` went with the path it gated, since it could not outlive it without keeping the coupling it measured. The **emit** is untouched — `data-astro-source-*` still goes out wherever Astro's compiler will not emit it, for the dev toolbar and other tooling. Driven live afterwards, the removal turned out to be stronger than the parity argument: **not one** legacy annotation survives to hover time on a real page — 0 of 1,231 on `01-example-site-2` (Astro 7.3.2), 0 of 409 on `sf-sf` (Astro 5.18.2) — while every tool-owned one does. The removed path was not a redundant fallback but an empty set. One user-visible consequence: `sourceAnnotations: 'off'` now disables the overlay on every version, and the startup warning says so
...
- [x] A committed fixture carries a genuine three-deep chain, and the guarantee is a test rather than a browser session — `/marketing`, four links from the route, asserted in `composition-render.test.ts` on both compilers
- [x] The client reads no annotation namespace but its own, proved on a real page rather than argued: 0 of 1,231 legacy annotations survive to hover time on Astro 7.3.2, 0 of 409 on Astro 5.18.2, against 1,231 and 309 tool-owned
