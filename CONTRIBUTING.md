...
- It writes nothing outside the confinement. Every *client-supplied* path goes through `validateEditablePath`, and every write is atomic and verified against the source the page actually showed. The one fixed target — the settings file — is a constant in server source rather than anything a request can name, and it is written `0600`.
