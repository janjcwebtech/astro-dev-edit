...
**Astro 5.x–7.x, two annotation regimes.** The whole feature rides on the `data-astro-source-file` / `-loc` attributes on every element in dev. On Astro 5/6 Astro's compiler emits them (dev toolbar required); Astro 7's Rust compiler (`@astrojs/compiler-rs`) does not ([withastro/compiler-rs#96](https://github.com/withastro/compiler-rs/issues/96)), so on ≥7 the integration **injects them itself** — `src/server/annotate.ts`, a pre-compiler Vite transform gated by the `sourceAnnotations` option. Its locs must stay loc-rule-identical to the patcher's (`tests/helpers.ts::locOf`); the transform must keep hook-level `transform: { order: 'pre' }`, because Astro's own `astro:build` plugin is also `enforce: 'pre'` and compiles `.astro` earlier in the array — plugin-level enforce alone would receive compiled JS. Full history and rationale in [`docs/ASTRO-COMPAT.md`](docs/ASTRO-COMPAT.md).
