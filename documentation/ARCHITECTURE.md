...
**One annotation regime.** The whole feature rides on source annotations present in dev, and the tool emits its own — `data-atx-file` / `-loc` — on every supported version rather than depending on Astro's, which needs the dev toolbar on 5/6 and is gone on 7. `src/server/annotate.ts` is that pre-compiler Vite transform; `sourceAnnotations: 'off'` is the opt-out. It emits nothing in Astro's `data-astro-source-*` namespace: where Astro emits its own, the compiler's (injection-shifted) loc wins the duplicate rather than losing it, and where Astro emits nothing the pair reaches only readers outside the tool, at one absolute path per element. Two invariants hold it: its locs stay loc-rule-identical to the patcher's (`tests/helpers.ts::locOf`), and it keeps **hook-level** `transform: { order: 'pre' }`, because plugin-level `enforce` alone is not enough.

Three layers meet at one type-only contract.

## Component tracing

The [tracing API](COMPOSITION-API.md) is always installed, and
...
- **No exception.** The tool emits nothing in Astro's `data-astro-source-*` namespace, so no seam puts an absolute path into a served page.
...
- The host never gains `transform`, `filter` or `contain`, which would re-anchor every `position: fixed` panel to it. It stays `display: contents`; the docked layout moves the page by padding `<html>`, never by giving the host a box.
...
`overlay.ts` is the entry point and does three things: it starts source-annotation capture, it puts the shadow root back after a `<ClientRouter />` swap, and once `/health` answers it hands everything to `inspector-app.ts`.
...
- **Selection is confirmed against the server.** The DOM cannot tell a resolved `{expression}` from literal text; only the AST can, so `/classify` decides what a value is and `value-rows.ts` turns that verdict into the rows the Values card shows. `editors/image.ts` is a section of that panel, not a modal.
- **`inspector-app.ts`** is the UI composition root. It holds Alt/Option interception and the selection as **independent lifetimes**: releasing the key hands clicks back to the page while the panel, its outline and the tree's selection stay. An open tree arms the same interception without the key (`armed()`), so it ends when the tree closes. `inspector.ts` draws the panel; `inspector-model.ts` is pure — the request generation that discards late responses, and the occurrence grouping that distinguishes repeated insertions.
...
| `editors/peek.ts`, `editors/media-grid.ts` | The floating-mode source preview, and the picker's grid |

Theming is `--atx-*` plus a deliberately small `::part()` set (`panel`, `drawer`, `backdrop`, `pill`, `toast`), assigned centrally from `PARTS` in `ui.ts::styled`. A new part is an API commitment — add one only on demand. Both tables are documented in [Styling reference](STYLING.md).
...
Vitest characterization tests pin patcher, middleware and composition behavior. The client layer's unit tests cover its pure modules — `tree-model.ts`, `inspector-model.ts`, `layout-model.ts`, `value-model.ts`, `value-rows.ts`, `element-context.ts`, `variable-tag.ts`, `classify-cache.ts`, `highlight.ts`, `asset-view.ts` and `shadow.ts`; the DOM half is covered by a manual checklist.
