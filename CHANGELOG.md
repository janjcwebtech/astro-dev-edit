...
- The inspector opens on the **Component chain** and puts **Values** under it, and grows a status row under its title bar: the chain's tier, whether the page still matches disk, and **Copy context** — this element's selector, source loc, chain and applied CSS as one paste.
...
- Every card in the overlay **collapses** from a chevron in its header, and the header is a band with a rule under it. Rules inside a card are inset to the content rather than running to its edges.
