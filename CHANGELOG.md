...
-   A **Settings panel** in the admin bar's overflow menu, holding the Unsplash access key. Storing it here writes `.astro-text-edit.json` at the project root with `0600` permissions, so the key never has to go near your committed config; the panel warns when that file is not covered by your `.gitignore`. The key is never sent back to the browser — a read reports only whether one resolved, from where, and a masked fragment. When a key is set in `astro.config.mjs` or the environment the field is disabled and says so, rather than accepting a value that resolution would ignore. New `atx-settings-*` styling hooks and a `settings` icon
